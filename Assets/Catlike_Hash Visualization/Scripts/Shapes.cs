...
public static class Shapes
{
    ...
    public struct Job<S> : IJobFor where S : struct, IShape
    {
        ...
        public float3x4 positionsTRS, normalsTRS;
        ...
        public void Execute(int i) 
        {
            ...
            float3x4 n = transpose(TransformVectors(normalsTRS, p.normals, 0f));
            normals[i] = float3x4(normalize(n.c0), normalize(n.c1), normalize(n.c2), normalize(n.c3));

            positions[i] = transpose(TransformVectors(positionsTRS, p.positions));
        }

        public static JobHandle ScheduleParallel(NativeArray<float3x4> positions, NativeArray<float3x4> normals, float4x4 trs, int resolution, JobHandle dependency)
        {
            float4x4 tim = transpose(inverse(trs));
            return new Job<S>
            {
                positionsTRS = float3x4(trs.c0.xyz, trs.c1.xyz, trs.c2.xyz, trs.c3.xyz),
                normalsTRS = float3x4(tim.c0.xyz, tim.c1.xyz, tim.c2.xyz, tim.c3.xyz),
                positions = positions,
                normals = normals,
